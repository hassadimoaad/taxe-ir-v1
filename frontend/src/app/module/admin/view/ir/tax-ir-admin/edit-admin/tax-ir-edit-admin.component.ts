...
@Component({
  selector: 'app-tax-ir-edit-admin',
  templateUrl: './tax-ir-edit-admin.component.html'
})
export class TaxIrEditAdminComponent extends AbstractEditController<TaxIrDto, TaxIrCriteria, TaxIrService>   implements OnInit {
   ...
   public addTaxeIrEmployes() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.taxeIrEmployesElement.id == null){
                this.item.taxeIrEmployes.push(this.taxeIrEmployesElement);
            }else{
                const index = this.item.taxeIrEmployes.findIndex(e => e.id == this.taxeIrEmployesElement.id);
                this.item.taxeIrEmployes[index] = this.taxeIrEmployesElement;
            }
